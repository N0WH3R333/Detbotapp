...
async def force_sync_products_from_json():
    ...
    async with pool.acquire() as connection:
        async with connection.transaction():
            # Шаг 1: Полностью очищаем таблицу
            ...
            for product in products_from_file:
                await connection.execute(
                    """
                    INSERT INTO products (id, name, price, category, subcategory, image_url, description, detail_images)
                    VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
                    """,
                    product.get('id'), product.get('name'), product.get('price'),
                    product.get('category'), product.get('subcategory'), product.get('image_url'),
                    product.get('description'), json.dumps(product.get('detail_images', []))
                )
